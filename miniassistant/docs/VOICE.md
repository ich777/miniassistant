...
MiniAssistant supports voice messages on Matrix and Discord via the Wyoming protocol.
...
- **TTS** (text-to-speech): synthesizes replies — recommended: [wyoming-piper](https://github.com/rhasspy/wyoming-piper)

Both are optional independently: STT-only → voice input with text reply. TTS-only is not useful.
...
voice:
  ...
  tts:
    url: tcp://localhost:10200      # Wyoming TTS server (optional)
  language: de                      # STT language hint (ISO 639-1)
  tts_voice: de_DE-thorsten-medium  # Piper voice (optional, uses server default if omitted)
```
