...
```bash
GOOS=linux  GOARCH=amd64   go build -o maclient-linux-amd64 .
GOOS=linux  GOARCH=arm64   go build -o maclient-linux-arm64 .
GOOS=darwin GOARCH=arm64   go build -o maclient-macos-arm64 .
GOOS=windows GOARCH=amd64  go build -o maclient.exe .
```
...
```bash
# Frage als Argument
maclient --question "Was ist die Hauptstadt von Frankreich?"
maclient -q "Erkläre diesen Fehler: segfault at 0x0"

# Pipe (für Scripting)
echo "Fasse diese Ausgabe zusammen:" | cat - build.log | maclient -q
cat error.log | maclient -q "Was bedeutet dieser Fehler?"
git diff | maclient -q "Schreib eine Commit-Message für diesen Diff"
```
...
| `maclient --sessions` | Alle Sessions auflisten |
| `maclient --continue` | Session auswählen und fortsetzen |
| `maclient --continue 2` | Session Nr. 2 direkt fortsetzen |
| `maclient --continue abc123` | Session per ID-Prefix fortsetzen |
| `maclient --question TEXT` | Einzel-Frage, Antwort auf stdout, beenden |
...
Konfigurierbar über `maclient config` (Schritt 5) oder manuell in der JSON-Datei.
...
`read_url` auf dem **Client-System** ausgeführt werden (konfigurierbar über `maclient config`):
